...
foreach ($app['hooks'] as $key=>$hook) {
    ...
    $keys = empty($hook['array_keys'])? 'null' : 'array("'.implode('", "', $hook['array_keys']).'")';
    $content = "<?php
...
class apps".ucfirst($app['id']).ucfirst(str_replace('.','',$key))."Handler extends waEventHandler
{
    ...
    {
        \$event = wa('apps')->event('".$app['id'].'.'.$key."', \$params, \$array_keys);
        return ifempty(\$event);
    }
}
";
    $data[] = waFiles::write($handlersPath.$handler, $content);
}
}
...
class appsFrontController extends waFrontController
{
        ...
        if ($env == 'backend') {
            ...
            if (empty($module) && empty($plugin)) {
                $routing = new waRouting($this->system, array(
                    'default' => array(
                        array(
                            'url' => wa()->getConfig()->systemOption('backend_url').'/apps/*',
                            'app' => 'apps',
                        ),
                    ),
                ));
    ...
    protected function runController($controller, $params = null)
    {
        ...
        $handled = wa('apps')->event('controller_before.'.$class, $evt_params);
        if ($handled) {
            return;
        }
        $result = parent::runController($controller, $params);
        wa('apps')->event('controller_after.'.$class, $params);
        return $result;
    }
}
